# 🍪 Idle Clicker Game# 🍪 Idle Clicker Game# 🍪 Idle Clicker Game



An addictive idle clicker game inspired by Cookie Clicker, built with React and Zustand.



## 🎮 [PLAY NOW!](https://jmt-code.github.io/Idle_game/)An addictive idle clicker game inspired by Cookie Clicker, built with React and Zustand.Un adictivo juego idle clicker estilo Cookie Clicker construido con React y Zustand.



> **Live Version:** https://jmt-code.github.io/Idle_game/



---## 🎮 [PLAY NOW!](https://jmt-code.github.io/Idle_game/)## 🎮 [¡JUEGA AHORA!](https://jmt-code.github.io/Idle_game/)



## 📖 How to Play



### 🎯 Objective> **Live Version:** https://jmt-code.github.io/Idle_game/> **Live Version:** https://jmt-code.github.io/Idle_game/

Bake as many cookies as you can by clicking the giant cookie and purchasing buildings that generate them automatically.



### 🖱️ Basic Controls

1. **Click the cookie** 🍪 - Generates +1 cookie per click------

2. **Buy buildings** 🏪 - Generates cookies automatically

3. **Upgrade your production** 📈 - Unlock more powerful buildings



### 🏪 Purchase System## 📖 How to Play## 📖 Cómo Jugar

- **Quantity selector**: Choose how many buildings to buy at once

  - `x1` - Buy 1 building

  - `x5` - Buy 5 buildings

  - `x10` - Buy 10 buildings### 🎯 Objective### 🎯 Objetivo

  - `x100` - Buy 100 buildings

  - `MAX` - Buy the maximum you can affordBake as many cookies as you can by clicking the giant cookie and purchasing buildings that generate them automatically.Hornea tantas galletas como puedas haciendo click en la galleta gigante y comprando edificios que las generen automáticamente.



### 🏗️ Available Buildings



| Building | Base Cost | Production/s | Description |### 🖱️ Basic Controls### 🖱️ Controles Básicos

|----------|-----------|--------------|-------------|

| 👆 Cursor | 15 | 0.1 | Autoclicks the cookie |1. **Click the cookie** 🍪 - Generates +1 cookie per click1. **Click en la galleta** 🍪 - Genera +1 galleta por click

| 👵 Grandma | 100 | 1 | A nice grandma to bake more cookies |

| 🌾 Farm | 1,100 | 8 | Grows cookie plants from cookie seeds |2. **Buy buildings** 🏪 - Generates cookies automatically2. **Compra edificios** 🏪 - Genera galletas automáticamente

| ⛏️ Mine | 12,000 | 47 | Mines out cookie dough and chocolate chips |

| 🏭 Factory | 130,000 | 260 | Produces large quantities of cookies |3. **Upgrade your production** 📈 - Unlock more powerful buildings3. **Mejora tu producción** 📈 - Desbloquea nuevos edificios más poderosos

| 🏦 Bank | 1,400,000 | 1,400 | Generates cookies from interest |

| ⛩️ Temple | 20,000,000 | 7,800 | Summons sacred cookies |

| 🧙 Wizard Tower | 330,000,000 | 44,000 | Summons cookies with magic spells |

| 🚀 Spaceship | 5,100,000,000 | 260,000 | Brings cookies from other planets |### 🏪 Purchase System### 🏪 Sistema de Compra

| ⏰ Time Machine | 75,000,000,000 | 1,600,000 | Brings cookies from the past |

- **Quantity selector**: Choose how many buildings to buy at once- **Selector de cantidad**: Elige cuántos edificios comprar a la vez

### 💡 Special Features

  - `x1` - Buy 1 building  - `x1` - Compra 1 edificio

#### 💾 Save System

- **Auto-save**: Automatically saves every 10 seconds  - `x5` - Buy 5 buildings  - `x5` - Compra 5 edificios

- **Export save**: 💾 Generate a code to share your progress

- **Import save**: 📥 Load a game from a code  - `x10` - Buy 10 buildings  - `x10` - Compra 10 edificios

- **Offline production**: Earn cookies even when not playing (max. 1 hour)

  - `x100` - Buy 100 buildings  - `x100` - Compra 100 edificios

#### 🎨 Visual Effects

- **Magic particles** ✨ when clicking  - `MAX` - Buy the maximum you can afford  - `MAX` - Compra el máximo que puedas con tus galletas

- **Glowing cookie** that changes based on your production:

  - 🟢 Green (>100 CPS)

  - 🟡 Gold (>1,000 CPS)

  - 🟣 Epic (>10,000 CPS)### 🏗️ Available Buildings### 🏗️ Edificios Disponibles

- **Achievement messages** when reaching important milestones

- **Informative tooltips** when hovering over buildings



#### 🎯 Milestone System| Building | Base Cost | Production/s | Description || Edificio | Costo Base | Producción/s | Descripción |

Receive special messages when reaching:

- 100 cookies 🎉|----------|-----------|--------------|-------------||----------|------------|--------------|-------------|

- 1,000 cookies 🎊

- 10,000 cookies 🌟| 👆 Cursor | 15 | 0.1 | Autoclicks the cookie || 👆 Cursor | 15 | 0.1 | Autoclicks para ti |

- 100,000 cookies 💫

- 1,000,000 cookies 🏆| 👵 Grandma | 100 | 1 | A nice grandma to bake more cookies || 👵 Abuela | 100 | 1 | Una abuela amable que hornea galletas |

- 10,000,000 cookies 👑

| 🌾 Farm | 1,100 | 8 | Grows cookie plants from cookie seeds || 🌾 Granja | 1,100 | 8 | Cultiva ingredientes para galletas |

### 🎮 Strategies and Tips

| ⛏️ Mine | 12,000 | 47 | Mines out cookie dough and chocolate chips || ⛏️ Mina | 12,000 | 47 | Extrae azúcar de las profundidades |

1. **Start by buying Cursors** - They're cheap and effective at the beginning

2. **Unlock new buildings** - You need to buy at least 1 of the previous building| 🏭 Factory | 130,000 | 260 | Produces large quantities of cookies || 🏭 Fábrica | 130,000 | 260 | Producción masiva de galletas |

3. **Use the MAX multiplier** - When you have many cookies, buy in bulk

4. **Take advantage of offline time** - The game keeps generating cookies (up to 1 hour)| 🏦 Bank | 1,400,000 | 1,400 | Generates cookies from interest || 🏦 Banco | 1,400,000 | 1,400 | Genera galletas mediante inversiones |

5. **Export your save** - Don't lose your progress, save your code

6. **Check the tooltips** - They show you exactly how much you'll improve| ⛩️ Temple | 20,000,000 | 7,800 | Summons sacred cookies || ⛩️ Templo | 20,000,000 | 7,800 | Galletas sagradas |



---| 🧙 Wizard Tower | 330,000,000 | 44,000 | Summons cookies with magic spells || 🧙 Torre de Mago | 330,000,000 | 44,000 | Invoca galletas mágicas |



## 🚀 Technical Features| 🚀 Spaceship | 5,100,000,000 | 260,000 | Brings cookies from other planets || 🚀 Nave Espacial | 5,100,000,000 | 260,000 | Encuentra galletas en otros planetas |



### ✨ Technologies| ⏰ Time Machine | 75,000,000,000 | 1,600,000 | Brings cookies from the past || ⏰ Máquina del Tiempo | 75,000,000,000 | 1,600,000 | Trae galletas del pasado |

- **React 18.3** - Reactive UI framework

- **Zustand 4.5** - Global state with persistence

- **Vite 5.4** - Ultra-fast build tool

- **CSS Animations** - Smooth visual effects### 💡 Special Features### 💡 Características Especiales



### 🎨 Development Features

- Save system with Base64 compression

- Offline production calculation based on timestamp#### 💾 Save System#### 💾 Sistema de Guardado

- Scalable cost formula (1.15x per unit)

- Automatic persistence with localStorage- **Auto-save**: Automatically saves every 10 seconds- **Auto-guardado**: Se guarda automáticamente cada 10 segundos

- Adaptive tooltips based on screen position

- Optimized CSS animations- **Export save**: 💾 Generate a code to share your progress- **Exportar partida**: 💾 Genera un código para compartir tu progreso



---- **Import save**: 📥 Load a game from a code- **Importar partida**: 📥 Carga una partida desde un código



## 💻 Local Installation- **Offline production**: Earn cookies even when not playing (max. 1 hour)- **Producción offline**: Ganas galletas incluso cuando no estás jugando (máx. 1 hora)



If you want to run the game on your machine:



### Requirements#### 🎨 Visual Effects#### 🎨 Efectos Visuales

- Node.js 16+ 

- npm or yarn- **Magic particles** ✨ when clicking- **Partículas mágicas** ✨ al hacer click



### Steps- **Glowing cookie** that changes based on your production:- **Galleta brillante** que cambia según tu producción:



```bash  - 🟢 Green (>100 CPS)  - 🟢 Verde (>100 CPS)

# Clone repository

git clone https://github.com/Jmt-code/Idle_game.git  - 🟡 Gold (>1,000 CPS)  - 🟡 Dorado (>1,000 CPS)

cd Idle_game

  - 🟣 Epic (>10,000 CPS)  - 🟣 Épico (>10,000 CPS)

# Install dependencies

npm install- **Achievement messages** when reaching important milestones- **Mensajes de logros** cuando alcanzas hitos importantes



# Run in development- **Informative tooltips** when hovering over buildings- **Tooltips informativos** al pasar el ratón por los edificios

npm run dev



# Build for production

npm run build#### 🎯 Milestone System#### 🎯 Sistema de Hitos



# Preview buildReceive special messages when reaching:Recibe mensajes especiales al alcanzar:

npm run preview

```- 100 cookies 🎉- 100 galletas 🎉



The game will be available at `http://localhost:3000`- 1,000 cookies 🎊- 1,000 galletas 🎊



---- 10,000 cookies 🌟- 10,000 galletas 🌟



## 🚀 Deploy to GitHub Pages- 100,000 cookies 💫- 100,000 galletas 💫



```bash- 1,000,000 cookies 🏆- 1,000,000 galletas 🏆

# Build and deploy

npm run deploy- 10,000,000 cookies 👑- 10,000,000 galletas 👑

```



The game will be automatically deployed to: `https://jmt-code.github.io/Idle_game/`

### 🎮 Strategies and Tips### 🎮 Estrategias y Consejos

---



## 📝 License

1. **Start by buying Cursors** - They're cheap and effective at the beginning1. **Empieza comprando Cursors** - Son baratos y efectivos al principio

This project is open source and available under the MIT license.

2. **Unlock new buildings** - You need to buy at least 1 of the previous building2. **Desbloquea edificios nuevos** - Necesitas comprar al menos 1 del edificio anterior

---

3. **Use the MAX multiplier** - When you have many cookies, buy in bulk3. **Usa el multiplicador MAX** - Cuando tengas muchas galletas, compra en masa

## 🎮 Have Fun Playing!

4. **Take advantage of offline time** - The game keeps generating cookies (up to 1 hour)4. **Aprovecha el tiempo offline** - El juego sigue generando galletas (hasta 1 hora)

**How many cookies can you bake? Start clicking and find out!** 🍪✨

5. **Export your save** - Don't lose your progress, save your code5. **Exporta tu partida** - No pierdas tu progreso, guarda tu código

---

6. **Check the tooltips** - They show you exactly how much you'll improve6. **Observa los tooltips** - Te muestran exactamente cuánto mejorarás

<div align="center">

  <p>Made with ❤️ and lots of 🍪</p>

  <p><strong><a href="https://jmt-code.github.io/Idle_game/">🎮 PLAY NOW</a></strong></p>

</div>------




## 🚀 Technical Features## � Características Técnicas



### ✨ Technologies### ✨ Tecnologías

- **React 18.3** - Reactive UI framework- **React 18.3** - Framework UI reactivo

- **Zustand 4.5** - Global state with persistence- **Zustand 4.5** - Estado global con persistencia

- **Vite 5.4** - Ultra-fast build tool- **Vite 5.4** - Build tool ultra rápido

- **CSS Animations** - Smooth visual effects- **CSS Animations** - Efectos visuales fluidos



### 🎨 Development Features### 🎨 Características de Desarrollo

- Save system with Base64 compression- Sistema de guardado con compresión Base64

- Offline production calculation based on timestamp- Cálculo de producción offline basado en timestamp

- Scalable cost formula (1.15x per unit)- Fórmula de costo escalable (1.15x por unidad)

- Automatic persistence with localStorage- Persistencia automática con localStorage

- Adaptive tooltips based on screen position- Tooltips adaptativos según posición en pantalla

- Optimized CSS animations- Animaciones CSS optimizadas



------



## 💻 Local Installation## � Instalación Local



If you want to run the game on your machine:Si quieres ejecutar el juego en tu máquina:



### Requirements### Requisitos

- Node.js 16+ - Node.js 16+ 

- npm or yarn- npm o yarn



### Steps### Pasos



```bash```bash

# Clone repository# Clonar repositorio

git clone https://github.com/Jmt-code/Idle_game.gitgit clone https://github.com/tu-usuario/Idle_game.git

cd Idle_gamecd Idle_game



# Install dependencies# Instalar dependencias

npm installnpm install



# Run in development# Ejecutar en desarrollo

npm run devnpm run dev



# Build for production# Construir para producción

npm run buildnpm run build



# Preview build# Previsualizar build

npm run previewnpm run preview

``````



The game will be available at `http://localhost:3000`El juego estará disponible en `http://localhost:3000`



------



## 🚀 Deploy to GitHub Pages## 🚀 Desplegar en GitHub Pages



```bash```bash

# Build and deploy# Compilar y desplegar

npm run deploynpm run deploy

``````



The game will be automatically deployed to: `https://jmt-code.github.io/Idle_game/`El juego se desplegará automáticamente en: `https://tu-usuario.github.io/Idle_game/`



------



## 📝 License## 📝 Licencia



This project is open source and available under the MIT license.Este proyecto es de código abierto y está disponible bajo la licencia MIT.



------



## 🎮 Have Fun Playing!## � ¡Diviértete Jugando!



**How many cookies can you bake? Start clicking and find out!** 🍪✨**¿Cuántas galletas puedes hornear? ¡Empieza a hacer click y descúbrelo!** 🍪✨



------



<div align="center"><div align="center">

  <p>Made with ❤️ and lots of 🍪</p>  <p>Hecho con ❤️ y muchas 🍪</p>

  <p><strong><a href="https://jmt-code.github.io/Idle_game/">🎮 PLAY NOW</a></strong></p>  <p><strong><a href="https://jmt-code.github.io/Idle_game/">🎮 JUGAR AHORA</a></strong></p>

</div></div>

